...
public:
    ...
    virtual void onRegist(uint32_t nSeqNo, uint32_t nResultCode) = 0;
    ...
    virtual void onBindProxy(uint32_t nSeqNo, uint32_t nUserId, uint32_t nProxyId, uint32_t ope, uint32_t rst) = 0;
    virtual void onFriend(uint32_t nSeqNo, uint32_t nUserId, uint32_t nOperateId, uint32_t ope, uint32_t rst) = 0;
    virtual void onGetProxyUserList(uint32_t nSeqNo, uint32_t nType, const list<IM::BaseDefine::UserInfo>& lsUser) = 0;
    virtual void onSendRedpack(uint32_t nSeqNo, uint32_t nResultCode, uint32_t nUserId, uint32_t nSessionId, uint32_t nPackId, IM::BaseDefine::SessionType nType) = 0;

    virtual void onPK10Place(uint32_t nSeqNo, uint32_t nResultCode, uint32_t nUserId) = 0;


    virtual void onPassword(uint32_t nSeqNo, uint32_t nUserId) = 0;
};
