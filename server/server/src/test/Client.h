...
{
public:
    CClient(const string& strName, const string& strPass, const string strDomain="http://192.168.2.120:8080");
    ~CClient();
public:
...
public:
    void connect();
    void close();
    uint32_t regist(const string strName, const string strPass, const string strNickname, const string strBind, const string strCode);
    uint32_t login(const string strName, const string strPass);
    uint32_t password(const string& strPass);
    ...
    uint32_t bind(string szTarget, uint32_t nTag=0);
    uint32_t friendReq(uint32_t nOperateId, uint32_t nOperateType);
    uint32_t getUserList();
    uint32_t place(uint32_t nToId, const string& strMsg);


    uint32_t sendRedpacket(uint32_t nToId, IM::BaseDefine::MsgType nType, uint32_t currency, uint32_t pack_num, uint32_t pack_type, string pack_content);
    uint32_t claimPacket(uint32_t pack_id);

public:
    virtual void onError(uint32_t nSeqNo, uint32_t nCmd, const string& strMsg);
    virtual void onConnect();
    virtual void onClose();
    virtual void onRegist(uint32_t nSeqNo, uint32_t nResultCode);
    ...
    virtual void onBindProxy(uint32_t nSeqNo, uint32_t nUserId, uint32_t nProxyId, uint32_t ope, uint32_t rst);
    virtual void onFriend(uint32_t nSeqNo, uint32_t nUserId, uint32_t nOperateId, uint32_t ope, uint32_t rst);
    virtual void onGetProxyUserList(uint32_t nSeqNo, uint32_t nType, const list<IM::BaseDefine::UserInfo>& lsUser);
    virtual void onSendRedpack(uint32_t nSeqNo, uint32_t nResultCode, uint32_t nUserId, uint32_t nSessionId, uint32_t nPackId, IM::BaseDefine::SessionType nType);

    virtual void onPK10Place(uint32_t nSeqNo, uint32_t nResultCode, uint32_t nUserId);

    virtual void onPassword(uint32_t nSeqNo, uint32_t nUserId);
