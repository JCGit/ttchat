...
public:
    ...
    net_handle_t connect(const string& strIp, uint16_t nPort, const string& strName, const string& strPass, IPacketCallback* pCallback);
...
public:
    ...
    uint32_t password(uint32_t nUserId, const string& strPass);
    ...
    uint32_t bind(uint32_t nUser, string szTarget, uint32_t nTag=0);
    uint32_t friendReq(uint32_t nUser, uint32_t nOperate, uint32_t operate_type);
    uint32_t getUserList(uint32_t nUser);

    uint32_t sendRedpacket(uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nType, uint32_t currency, uint32_t pack_num, uint32_t pack_type, string pack_content);
    uint32_t claimPacket(uint32_t nUserId, uint32_t pack_id);
    uint32_t place(uint32_t nFromId, uint32_t nToId, const string& strMsg);
...
private:
    ...
    void _HandleBindProxyResponse(CImPdu* pPdu);
    void _HandleFriendOperateResponse(CImPdu* pPdu);
    void _HandleFriendOperateNotify(CImPdu* pPdu);
    ...
    void _HandleClaimRedpackNotify(CImPdu* pPdu);
    void _HandlePK10PlaceResponse(CImPdu* pPdu);
    void _HandleModifyResponse(CImPdu* pPdu);
private:
	bool 		m_bOpen;
