...
{
    Debug(__func__ << ": connection type " << conn_type)
    this->m_handle = handle;
    this->m_conn_type = conn_type;
    ConnMap_t* conns;
    if (conn_type == LOGIN_CONN_TYPE_CLIENT) {
        conns = &g_client_conn_map;
    } else {
        conns = &g_msg_serv_conn_map;
    }
    conns->insert(std::make_pair(handle, this));
    ::netlib_option(handle, NETLIB_OPT_SET_CALLBACK, (void*)imconn_callback);
    ::netlib_option(handle, NETLIB_OPT_SET_CALLBACK_DATA, (void*)conns);
}
...
{
	printf("handler:%d.", pPdu->GetCommandId());
...
void CLoginConn::_HandleMsgServRequest(CImPdu* pPdu)
{
	...
	log("======CLoginConn::_HandleMsgServRequest======");
