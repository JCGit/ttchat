...
static PhoneCode_t g_phone_code_map;  

string g_update_version;
string g_update_log;
...
void init_http_conn(const char* pszVer/*=NULL*/,const char* pszLog/*=NULL*/)
{
	netlib_register_timer(http_conn_timer_callback, NULL, 1000);

    g_update_version = string(pszVer);
    g_update_log = string(pszLog);
}
...
void CHttpConn::OnRead()
{
	...
	if (m_cHttpParser.IsReadAll()) {
		...
		}else if(strncmp(url.c_str(), "/version", 8) == 0){
            Json::Value value;
            value["ver"] = g_update_version;
            value["msg"] = g_update_log;
