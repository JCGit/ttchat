...
void CRouteServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
		...
		case CID_GROUP_PK10_PLACE_REQUEST:
			handlePK10PlaceRequest(pPdu);
			break;
        default:
            log("http_msg_server server, wrong cmd id=%d", pPdu->GetCommandId());
	}
}

void CRouteServConn::handlePK10PlaceRequest(CImPdu* pPdu)
{
	...
	if (pConn){
        pPdu->SetCommandId(IM::BaseDefine::CID_GROUP_PK10_PLACE_HD_REQUEST);
		pConn->SendPdu(pPdu);
	}
}
