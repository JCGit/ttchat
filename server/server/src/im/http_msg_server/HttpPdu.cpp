...
char* PackGetBCResult(uint32_t result, std::map<std::string, Json::Value>& info_list)
{
	Json::Value json_obj;
    ...
    if (info_list.size() > 0) {
		...
		std::map<std::string, Json::Value>::iterator it = info_list.begin();
		for (; it!=info_list.end(); ++it)
		{
			Json::Value info_obj;
            info_obj.append(it->first);
            info_obj.append(it->second);

            info_array.append(info_obj);
		}
        json_obj["info_list"] = info_array;
    }
    std::string json_str = json_obj.toStyledString();
    uint32_t content_len = json_str.size();
