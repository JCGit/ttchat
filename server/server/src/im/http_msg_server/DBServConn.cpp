...
void CDBServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_GROUP_CHANGE_MEMBER_RESPONSE:
            ...
            break; 
        case CID_GROUP_CHANGE_PK10_RULES_RESPONSE:
            _HandlePkGroupChangeRsp(pPdu);
            break;
        case CID_GROUP_PK10_PLACE_HD_REQUEST:
            _HandlePkGroupPlaceReq(pPdu);
            break;
        case CID_GROUP_PK10_PLACE_HD_RESPONSE:
            _HandlePkGroupPlaceRsp(pPdu);
            break;
        case CID_GROUP_PK10_RULES_RESPONSE:
            _HandlePkGroupResp(pPdu);
            break;
        default:
...
void CDBServConn::_HandlePkGroupResp(CImPdu* pPdu)
{
    IM::Group::IMGroupPK10RuleRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

    uint32_t nSize = msg.pk_group_size();
    for (uint32_t i=0; i!=nSize; ++i)
    {
        const IM::BaseDefine::GroupPKInfo& info = msg.pk_group(i);

        uint32_t nUserId = info.user_id();
        uint32_t nIsAdmin = info.is_admin();
        uint32_t nGroupId = info.group_id();
        string rules = info.rules();

        log("[http]receive user:%d initial pk10 group rules.", nUserId);

        Json::Reader reader;
        Json::Value value;
        if(!reader.parse(rules, value)){
            log("json parse error");
            return;
        }else{
            PK10Model::getInstance()->setProxyRule(nUserId, nIsAdmin, nGroupId, value);
        }
    }
}

void CDBServConn::_HandlePkGroupChangeRsp(CImPdu* pPdu)
{
    IM::Group::IMGroupChangePK10Rsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

    uint32_t nUserId = msg.user_id();
    uint32_t nIsAdmin = msg.is_admin();
    string rules = msg.pk10_rules();
    uint32_t nGroupId = msg.group_id();

    log("[http]receive user:%d change pk10 group rules.", nUserId);
...
void CDBServConn::_HandlePkGroupPlaceReq(CImPdu* pPdu)
{
	...
	uint32_t nUserId = msg.user_id();
	uint32_t nGroupId = msg.group_id();
    uint32_t nRule = msg.rule();
    uint32_t nTime = msg.time();

    const string& attach_data = msg.attach_data();

	list<COMB> lsComb;
	// 单次下注列表
	for (int i=0; i!=msg.items_size(); ++i)
	{
		const IM::Group::IMPlaceItem& place = msg.items(i);

		COMB comb;
		comb.main = place.main();
		comb.sub = place.sub();
		comb.gold = place.gold();

		lsComb.push_back(comb);
	}

    uint32_t nRet = PK10Model::getInstance()->place(nUserId, nGroupId, nTime, nRule, lsComb);

    CRouteServConn* pRouteConn = get_route_serv_conn();
    if (pRouteConn) {
        IM::Group::IMGroupPK10PlaceRsp msgResp;

        msgResp.set_user_id(nUserId);
        msgResp.set_result_code(nRet);
        msgResp.set_attach_data(attach_data);

        CImPdu pdu;
        pdu.SetPBMsg(&msgResp);
        pdu.SetServiceId(SID_GROUP);
        pdu.SetCommandId(CID_GROUP_PK10_PLACE_RESPONSE);
        pRouteConn->SendPdu(&pdu);
    }
}

void CDBServConn::_HandlePkGroupPlaceRsp(CImPdu* pPdu)
{
    CRouteServConn* pRouteConn = get_route_serv_conn();
    if (pRouteConn) {

        pPdu->SetCommandId(CID_GROUP_PK10_PLACE_RESPONSE);
