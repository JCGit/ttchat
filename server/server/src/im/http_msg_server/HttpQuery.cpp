...
void CHttpQuery::DispatchQuery(std::string& url, std::string& post_data, CHttpConn* pHttpConn)
{
    ...
    else if (strcmp(url.c_str(), "/query/BocaiRecord") == 0)
    {
        _QueryBocaiRecord(strAppKey, value, pHttpConn);
    }
    else if (strcmp(url.c_str(), "/query/pk10Place") == 0)
    {
...
void CHttpQuery::_QueryBocaiRecord(const string& strAppKey,Json::Value& post_json_obj, CHttpConn* pHttpConn)
{
    try
    {
        uint32_t pk10_start = post_json_obj["start"].asUInt();
        uint32_t pk10_line = post_json_obj["line"].asUInt();

        map<string, Json::Value> fieldInfo;
        pull_pk10_request(fieldInfo, pk10_start, pk10_line);

        char* response_buf = PackGetBCResult(0, fieldInfo);
        pHttpConn->Send(response_buf, (uint32_t)strlen(response_buf));
        pHttpConn->Close();
    }
    catch (std::runtime_error msg)
