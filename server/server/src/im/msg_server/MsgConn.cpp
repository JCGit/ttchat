...
using MsgConn = CMsgConn;
using DBServConn = CDBServConn;
using PduAttachData = CPduAttachData;
...
void CMsgConn::HandlePdu(CImPdu* pPdu) noexcept try {
    ...
    Debug(__func__ << " command id " << pPdu->GetCommandId())
	switch (pPdu->GetCommandId()) {
        case CID_OTHER_HEARTBEAT:
            _HandleHeartBeat(pPdu);
            break;
		case CID_LOGIN_REQ_REGISTER:
			_HandleUserRegister(pPdu);
            break;
        ...
        case CID_BUDDY_LIST_CHANGE_SIGN_INFO_REQUEST:
            ...
            break;
        case CID_BUDDY_LIST_CHANGE_INFO_REQUEST:
            _HandleChangeUserInfoRequest(pPdu);
            break;
        ...
        case CID_BUDDY_LIST_DEPARTMENT_REQUEST:
            ...
            break;
        case IM::BaseDefine::GroupCmdID::CID_GROUP_INFO_MODIFY_REQUEST:
            this->handleCID_GROUP_INFO_MODIFY_REQUEST(*pPdu);
            break;
        case IM::BaseDefine::GroupCmdID::CID_GROUP_SHIELD_REQUEST:
            this->handleCID_GROUP_SHIELD_REQUEST(*pPdu);
            break;
        // for group process
        ...
        case CID_GROUP_SHIELD_GROUP_REQUEST:
            ...
            break;
        case CID_GROUP_CHANGE_PK10_RULES_REQUEST:
            s_group_chat->HandlePK10GroupRuleRequest(pPdu, this);
            break;
        case CID_GROUP_PK10_PLACE_REQUEST:
            s_group_chat->HandlePK10PlaceRequest(pPdu, this);
            break;
        ...
        case CID_FILE_DEL_OFFLINE_REQ:
            ...
            break;
        // Buddy
        case IM::BaseDefine::BuddyListCmdID::CID_BUDDY_LIST_PUB_USER_INFO_REQUEST:
            this->handleCID_BUDDY_LIST_PUB_USER_INFO_REQUEST(*pPdu);
            break;
        // Proxy
        case IM::BaseDefine::ProxyCmdID::CID_PROXY_ENABLE_REQ:
            this->handleCID_PROXY_ENABLE_REQ(*pPdu);
        case CID_PROXY_BIND_REQ:
            _HandleBindProxyRequest(pPdu);
            break;
        case CID_PROXY_USER_LIST_REQ:
            _HandleUserListRequest(pPdu);
            break;
        case CID_FRIEND_OPERATE_REQ:
            _HandleFriendOperateRequest(pPdu);
            break;

        // for redpack
        case CID_REDPACK_SEND_REQ:
            _HandleRedpackSendRequest(pPdu);
            break;
        case CID_REDPACK_CLAIM_REQ:
            _HandleClaimRedpackRequest(pPdu);
            break;
        default:
            Warning("wrong msg, cmd id " << pPdu->GetCommandId()
                << " user id " << GetUserId())
            break;
	}
...
void CMsgConn::_HandleUserRegister(CImPdu* pPdu)
{
    ...
    string szName = msg.user_name();
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserByLoginName(szName);
    if (!pImUser) {
        pImUser = new CImUser(szName);
        CImUserManager::GetInstance()->AddImUserByLoginName(szName, pImUser);
    }
    ...
    pdu.SetPBMsg(&msg);
    pdu.SetServiceId(SID_LOGIN);
    pdu.SetCommandId(CID_LOGIN_REQ_REGISTER);
    pdu.SetSeqNum(pPdu->GetSeqNum());
    pDbConn->SendPdu(&pdu);
}
...
void CMsgConn::_HandleChangeUserInfoRequest(CImPdu* pPdu)
{
    IM::Buddy::IMChangeInfoReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log("_HandleChangeUserInfoRequest, user_id=%u ", GetUserId());
    CDBServConn* pDBConn = get_db_serv_conn();
    if (pDBConn) {
        msg.set_user_id(GetUserId());
        CPduAttachData attach(ATTACH_TYPE_HANDLE, m_handle,0, NULL);
...
// Buddy
void MsgConn::handleCID_BUDDY_LIST_PUB_USER_INFO_REQUEST(Packet& pkt)
noexcept try {
    Trace0()
    IM::Buddy::IMPublicUserInfoRequest msg;
    Assert(msg.ParseFromArray(pkt.GetBodyData(), pkt.GetBodyLength()),
        "ParseFromArray fail")
    DBServConn* const dbConn = ::get_db_serv_conn();
    Assert(dbConn, "get_db_serv_conn fail")
    CPduAttachData attach(ATTACH_TYPE_HANDLE, this->m_handle, 0, nullptr);
    msg.set_attach_data(attach.GetBuffer(), attach.GetLength());
    pkt.SetPBMsg(&msg);
    dbConn->SendPdu(&pkt);
} catch(std::exception const& e) {
...
// Proxy
void MsgConn::handleCID_PROXY_ENABLE_REQ(Packet& pkt) noexcept try {
...
// Group
void MsgConn::handleCID_GROUP_INFO_MODIFY_REQUEST(Packet& pkt) noexcept try {
    IM::Group::InfoModifyRequest msg;
    Assert(msg.ParseFromArray(pkt.GetBodyData(), pkt.GetBodyLength()),
        "ParseFromArray fail " << pkt.GetBodyLength())
    Assert(this->GetUserId() == msg.user_id(),
        "session user id != msg.user_id")
    DBServConn* const dbConn = ::get_db_serv_conn();
    Assert(dbConn, "get_db_serv_conn fail")
    PduAttachData attach(ATTACH_TYPE_HANDLE, this->m_handle, 0, nullptr);
    msg.set_attach(attach.GetBuffer(), attach.GetLength());
    pkt.SetPBMsg(&msg);
    dbConn->SendPdu(&pkt);
} catch(std::exception const& e) {
    Error0(e.what())
}
void MsgConn::handleCID_GROUP_SHIELD_REQUEST(Packet& pkt) noexcept try {
    IM::Group::ShieldRequest msg;
    Assert(msg.ParseFromArray(pkt.GetBodyData(), pkt.GetBodyLength()),
        "ParseFromArray fail")
    Assert(this->GetUserId() == msg.user_id(),
        "session user id != msg.user_id")
...
void CMsgConn::_HandleBindProxyRequest(CImPdu* pPdu)
{
    IM::Proxy::IMProxyBindReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log("_HandleBindProxyRequest, user:%d bind user:%s.", msg.user_id(), msg.user_bind().c_str());
...
void CMsgConn::_HandleFriendOperateRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleUserListRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleRedpackSendRequest(CImPdu* pPdu)
{
    ...
    {
        log("_HandleRedpackSendRequest from_user_id == to_user_id.");
        return;
    }
...
void CMsgConn::_HandleClaimRedpackRequest(CImPdu* pPdu)
{
