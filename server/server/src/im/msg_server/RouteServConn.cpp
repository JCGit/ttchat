...
void CRouteServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_GROUP_CHANGE_MEMBER_NOTIFY:
            ...
            break;
        case CID_GROUP_CHANGE_PK10_RULES_NOTIFY:
            s_group_chat->HandlePK10GroupRuleBroadcast(pPdu);
            break;
        case CID_FILE_NOTIFY:
            s_file_handler->HandleFileNotify(pPdu);
            break;
        case CID_GROUP_PK10_PLACE_RESPONSE:
            _HandleGroupPkPlaceResponse(pPdu);
            break;
        default:
...
void CRouteServConn::_HandleGroupPkPlaceResponse(CImPdu* pPdu)
{
    ...
    CMsgConn* pMsgConn = CImUserManager::GetInstance()->GetMsgConnByHandle(nUserId, handle);
    ...
    if (pMsgConn && pMsgConn->IsOpen()) {
        ...
        log("_HandleGroupPkPlaceResponse: can't found msg_conn by user_id = %u, handle = %u", nUserId, handle);
    }
}