...
void CGroupChat::HandlePK10GroupRuleRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
    IM::Group::IMGroupChangePK10Req msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

    uint32_t user_id = pFromConn->GetUserId();
    log("HandlePK10GroupRuleRequest, user_id=%u. ", user_id);
...
void CGroupChat::HandlePK10GroupRuleBroadcast(CImPdu* pPdu)
{
    IM::Group::IMGroupChangePK10Notify msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

    uint32_t nSize = msg.user_list_size();
    for (uint32_t i = 0; i < nSize; i++)
    {
        uint32_t user_id = msg.user_list(i);
        _SendPduToUser(pPdu, user_id);
    }
}

void CGroupChat::HandlePK10PlaceRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
...
void CGroupChat::HandleClientGroupCreateRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
    ...
    // 禁用临时群
    if (group_type == IM::BaseDefine::GROUP_TYPE_TMP) {
        log("HandleClientGroupCreateRequest, create tmp group failed, req_id=%u, group_name=%s. ", req_user_id, group_name.c_str());
        return;
    }
...
void CGroupChat::HandleGroupPK10PlaceResponse(CImPdu* pPdu)
{
    IM::Group::IMGroupPK10PlaceRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
