...
public:
    ...
    /**
     * @note after Close() all instance will be release when need
     * Like shared ptr -- ref count when delete this when need, but Close()
     * unsafe
     */
	...
	virtual void HandlePdu(CImPdu* pPdu) noexcept override;

	void AddToSendList(uint32_t msg_id, uint32_t from_id);
	void DelFromSendList(uint32_t msg_id, uint32_t from_id);
private:
    // Buddy
    void handleCID_BUDDY_LIST_PUB_USER_INFO_REQUEST(Packet& pkt) noexcept;
    // Proxy
    void handleCID_PROXY_ENABLE_REQ(Packet& pkt) noexcept;
    // Group
    /// Client -> msg server
    void handleCID_GROUP_INFO_MODIFY_REQUEST(Packet& pkt) noexcept;
    void handleCID_GROUP_SHIELD_REQUEST(Packet& pkt) noexcept;
    void _HandleHeartBeat(CImPdu* pPdu);
	void _HandleLoginRequest(CImPdu* pPdu);
	void _HandleUserRegister(CImPdu* pPdu);
    ...
    void _HandleChangeUserInfoRequest(CImPdu* pPdu);
    ...
    void _HandleBindProxyRequest(CImPdu* pPdu);
    void _HandleUserListRequest(CImPdu* pPdu);
    void _HandleFriendOperateRequest(CImPdu* pPdu);

    void _HandleRedpackSendRequest(CImPdu* pPdu);
    void _HandleClaimRedpackRequest(CImPdu* pPdu);
