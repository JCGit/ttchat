    ...
    if(CGroupModel::getInstance()->isInGroup(nFromId, nGroupId))
    {
        ...
        if (pDBConn)
        {
            ...
            if (pStmt->Init(pDBConn->GetMysql(), strSql))
            {
                ...
                if (bRet)
                {
                    /// FIXED: use captured dbconn
                    /// FIXME: after stmt ok??
                    //CGroupModel::getInstance()->updateGroupChat(nGroupId);
                    CGroupModel::getInstance()->modGroupChatTime(nGroupId, pDBConn);
...
bool CGroupMessageModel::sendRedpack(uint32_t nFromId, uint32_t nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, uint32_t currency, uint32_t number, uint32_t openRule, string& szText)
{
    Json::Value obj;
    
    obj["t_currency"] = currency;
    obj["t_number"] = number;
    obj["t_openrule"] = openRule;
    obj["t_text"] = szText;
    obj["t_msgtype"] = nMsgType;
    obj["t_session"] = nGroupId;
    obj["t_sender"] = nFromId;

    std::string szContent = obj.toStyledString();
	return sendMessage(nFromId, nGroupId, nMsgType, nCreateTime, nMsgId, szContent);
}
...
{
    std::set<uint32_t> gs;
    CGroupModel::getInstance()->fetUGrpids(nUserId, gs, 0);
    uint32_t nCount = 0;
...
//                log("hget %s : count failed !", strGroupKey.c_str());
            ...
            string strUserKey = int2string(nUserId) + "_" + int2string(groupId) + GROUP_USER_MSG_COUNTER_REDIS_KEY_SUFFIX;
            ...
            if(nCount > 0)
            {
                ...
                cUnreadInfo.set_session_id(groupId);
                ...
                uint32_t nFromId;
                getLastMsg(groupId, nMsgId, strMsgData, nType, nFromId);
                ...
                {
                    log("invalid msgType. userId=%u, groupId=%u, msgType=%u, msgId=%u", nUserId, groupId, nType, nMsgId);
                }
            }
        }
        pCacheManager->RelCacheConn(pCacheConn);
