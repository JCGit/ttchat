...
void QueryPublicUserInfo(Packet* pkt, uint32_t connUuid) noexcept try {
    Trace0()
    IM::Buddy::IMPublicUserInfoRequest msg;
    Assert(msg.ParseFromArray(pkt->GetBodyData(), pkt->GetBodyLength()),
        "pb fail")
    int32_t responseCode;
    std::string const key = msg.key();
    uint32_t const reqUserId = msg.user_id();
    std::vector<IM::BaseDefine::UserInfo> userInfos;
    if (key.empty()) {
        Warning(__func__ << ": empty key")
        responseCode = -1;// empty key
    } else {
        uint64_t const keyFlags = msg.key_flags();
        responseCode = UserModel::getInstance()->queryPublicUserInfo(
            reqUserId, key, keyFlags, userInfos);
    }
    IM::Buddy::IMPublicUserInfoResponse resp;
    resp.set_user_id(reqUserId);
    resp.set_code(responseCode);
    if (msg.has_attach_data()) {
        resp.set_attach_data(msg.attach_data());
    }
    for (auto&& userInfo : userInfos) {
        auto* const p = resp.add_user_info();
        *p = std::move(userInfo);
    }
    Packet* const respPkt = new Packet();
    Assert(respPkt, "no memory")
    respPkt->SetPBMsg(&resp);
    respPkt->SetSeqNum(pkt->GetSeqNum());
    respPkt->SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
    respPkt->SetCommandId(IM::BaseDefine::BuddyListCmdID
        ::CID_BUDDY_LIST_PUB_USER_INFO_RESPONSE);
    ProxyConn::AddResponsePdu(connUuid, respPkt);
...
msgResp.set_user_id(from_user_id);
    ...
    //            *pUser = *it;
                ...
                pUser->set_binded(it->binded());
                pUser->set_member_order(it->member_order());
    ...
    void changeUserInfo(CImPdu* pPdu, uint32_t conn_uuid)
    {
        IM::Buddy::IMChangeInfoReq req;
        if(req.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {

            DBUserInfo_t user;
            user.nId = req.user_id();
            user.nSex = req.sex();
            user.strNick = req.nick_name();
            user.nBirthday = req.birthday();

            bool result = false;

            if (req.has_password()){
                result = CUserModel::getInstance()->updateUserPassword(req.user_id(), req.password());
            }else{
                result = CUserModel::getInstance()->updateUserInfo(user);
            }

            IM::Buddy::IMChangeInfoRsp resp;
            resp.set_user_id(user.nId);
            resp.set_result_code(result ? 0 : 1);
            resp.set_attach_data(req.attach_data());
            log("[info]user:%s change info result:%d.", int2string(user.nId).c_str(), resp.result_code());
            
            CImPdu* pdu_resp = new CImPdu();
            pdu_resp->SetPBMsg(&resp);
            pdu_resp->SetSeqNum(pPdu->GetSeqNum());
            pdu_resp->SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
            pdu_resp->SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_CHANGE_INFO_RESPONE);
            CProxyConn::AddResponsePdu(conn_uuid, pdu_resp);
        
        } else {
            log("changeUserInfo: IMChangeInfoReq ParseFromArray failed!!!");
        }
    }

    void changeUserAvatar(CImPdu* pPdu, uint32_t conn_uuid)
    {
       ...
       if(req.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())) {
            ...
            DBUserInfo_t user;
            user.nId = req.user_id();
            user.strAvatar = req.avatar_url();

            bool result = CUserModel::getInstance()->updateUserInfo(user);
            
            IM::Buddy::IMChangeAvatarRsp resp;
            resp.set_user_id(user.nId);
            resp.set_result_code(result ? 0 : 1);
            resp.set_attach_data(req.attach_data());
            log("[info]user:%s change avatar result:%d.", int2string(user.nId).c_str(), resp.result_code());
            ...
            log("changeUserAvatar: IMChangeAvatarReq ParseFromArray failed!!!");
        }
    }
