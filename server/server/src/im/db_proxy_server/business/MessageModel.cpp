 ...
 */
bool CMessageModel::sendMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, string const& strMsgContent)
{
    bool bRet =false;
...
bool CMessageModel::sendRedpack(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime,
                    uint32_t nMsgId, uint32_t currency, uint32_t number, uint32_t openRule, string& szText)
{
    Json::Value obj;
	...
	return sendMessage(nRelateId, nFromId, nToId, nMsgType, nCreateTime, nMsgId, szContent);
}
