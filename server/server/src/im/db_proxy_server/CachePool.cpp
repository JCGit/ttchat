...
CacheConn* CachePool::GetCacheConn()
{
	...
	while (m_free_list.empty()) {
		if (m_cur_conn_cnt >= m_max_conn_cnt) {
			log("CachePool current no cache connection for %s.", m_pool_name.c_str());
			m_free_notify.Wait();
		} else {
...
std::shared_ptr<CacheConn> CachePool::GetCacheConnPtr()
{
	std::shared_ptr<CacheConn> cacheptr;

	auto threadId = std::this_thread::get_id();
	// 限制单线程 只有一个引用
	{
		std::lock_guard<std::mutex> cachelock(m_thread_mutex);

		if (m_cache_threads.find(threadId) != m_cache_threads.end()){
			return nullptr;
		}
		m_cache_threads.insert(threadId);
	}
	
	CacheConn* pCacheConn = GetCacheConn();
	cacheptr.reset(pCacheConn, [this, threadId](CacheConn*& p){
		// shared_ptr<CacheConn> 析构
		if (p){
			RelCacheConn(p);
			p = NULL;
			...
			{
				std::lock_guard<std::mutex> cachelock(m_thread_mutex);
				m_cache_threads.erase(threadId);
			}
		}
	});
}
...
std::shared_ptr<CacheConn> CacheManager::GetCacheConnPtr(const char* pool_name)
{
	return std::shared_ptr<CacheConn>(GetCacheConn(pool_name));
}
