...
void CHttpConn::OnRead()
{
    ...
    static uint64_t __start = 0;

    if (bRecvInit == false){
        bRecvInit = true;
        __start = get_tick_count();
    }
    
    if (m_HttpParser.IsReadAll())
    {
        uint64_t __end = get_tick_count();
        float cost = (__end - __start) * 1.0 / 1000;
        float file = m_in_buf.GetWriteOffset() / 1024;
        ...
        log("file:%fkb, start:%lld, end:%lld, cost:%fs speed:%fkb/s.", file, __start, __end, cost, speed);

        bRecvInit = false;
    }else{
        log("msfs receive offset:%fkb.", m_in_buf.GetWriteOffset() * 0.0001);
    }
}
