...
#include "uchat/rwlock.hpp"
namespace uchat
{
/// Auto unlock guard for read lock
struct ScopedReadLock {
    ScopedReadLock(Rwlock& lock): lock(lock) { this->lock.rdlock(); }
    virtual ~ScopedReadLock() { this->lock.unlock(); }
    // do not try copy
...
struct ScopedWriteLock {
    ScopedWriteLock(Rwlock& lock): lock(lock) { this->lock.wrlock(); }
    virtual ~ScopedWriteLock() { this->lock.unlock(); }
    // do not try copy
...
//    BoostScopedWriteLock(boost::shared_mutex& rwlock):
//        uplock(rwlock), writeLock(uplock) {}
