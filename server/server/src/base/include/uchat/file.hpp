...
#include <memory>
#include <functional>
#include "uchat/scopedrwlock.hpp"
#include "uchat/flags.hpp"
namespace uchat
{
// Some file related specs.
static_assert(sizeof(long) <= 8, "support 64 or 32");
constexpr char const* kFileIoLogName = "__uchat_file_io__";
/**
 ...
 */
extern int64_t GetFileSize(std::string const& filename) noexcept;
/**
 * Get file size
 * @param fd file fd
 * @return >= 0: file size when successfully else -errno
 * @note POSIX.1 规定 stat 结构的 st_size 成员对于管道是未定义的.
 * 但是当 fstat 函数应用于管道读端的文件描述符时 很多系统在 st_size 中存放可用于读的
 * 字节数. 但是, 这是不可移植的.
 * @sa
 * - man 2 fstat
 * - UNIX 环境高级编程
 */
extern int64_t GetFileSize(int const fd) noexcept;
/// @return bytes in atomic write to a pipe
static constexpr inline int32_t GetPipeBufSize() noexcept
{
    return PIPE_BUF;
}
/**
 ...
 * @param filename file name
 * @return 1 when file exists, 0 when not,  -errno (<0) when error
 */
extern int IsExists(std::string const& filename) noexcept;
/**
 * Check whether file is directory
 * @param filename file name
 * @return 1 when file is directory, 0 when not,  -errno (<0) when error
 */
extern int IsDirectory(std::string const& filename) noexcept;
/**
 * Check whether file is pipe
 * @param fd file fd
 * @return 1 when file is pipe or fifo, 0 when not,  -errno (<0) when error
 * @sa
 * - man 2 fstat
 * - UNIX 环境高级编程: fstat 函数对管道的每一端都返回一个 FIFO 类型的文件描述符,
 *   可以用 S_ISFIFO 宏来测试管道
 */
extern int IsPipe(int const fd) noexcept;
/**
 ...
 * @sa remove
 */
extern int RemoveFiles(std::string const& pathname) noexcept;
extern int RemoveFile(std::string const& pathname) noexcept;
extern int RemoveFileBySystem(std::string const& pathname) noexcept;
/**
 * Create directories
...
struct FileOpenConfig {
    ...
    /// 获取文件创建权限
    inline SharedPtr<PosixFileModes> getMode() const noexcept {
        return this->mode; }
private:
    int flags;///< 文件打开 flags
    SharedPtr<PosixFileModes> mode{ nullptr };///< 文件创建权限
};
/**
 * @struct File
 * 文件 IO
...
struct File {
     ...
     * @return >= 0 when succes else -errno
     */
    int open(FileOpenConfig const& openConfig) noexcept;
    /// Close this file
    void close() noexcept;
    /**
     ...
     */
    std::tuple<int, SharedPtr<std::vector<uint8_t> > > read(
    ...
    std::tuple<int, size_t> write(
        ...
        ssize_t const limit = -1) noexcept;
    static int open(std::string const& filename, int const flag) noexcept;
    static int open(
        std::string const& filename,
        int const flag,
...
private:
    ...
    ssize_t __size() const noexcept;
    //mutable boost::shared_mutex filenameRwlock;
    //mutable boost::shared_mutex fdRwlock;
    mutable Rwlock filenameRwlock;
    mutable Rwlock fdRwlock;
