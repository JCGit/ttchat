...
Rwlock Logger::instancesRwlock;
Logger& Logger::hasLogger(std::string const& file) noexcept
{
    BoostScopedReadLock reaLock(Logger::instancesRwlock);
...
std::map<std::string, std::shared_ptr<Logger> > Logger::instances;
Logger Logger::emptyLogger("");
Logger& Logger::getLogger(
    LogLevel const& spinOnceLogLevel,
    ...
    Outputs const& outputs) noexcept
{
    ...
    if (&Logger::emptyLogger != &l) {
        ...
        if (outputs.testFlag(Output::CoutOrCerr)
            || outputs.testFlag(Output::File)) {
            l.outputs = outputs;
        }
        return l;
    }
    BoostScopedWriteLock writeLock(Logger::instancesRwlock);
    auto const ret = std::shared_ptr<Logger>(new Logger(file, outputs));
...
Logger::Logger(
    ...
    bool const trunc) noexcept: path(path)
{
    ...
    if (auto const ac = this->appendCallback) {
        {
            ...
            if (this->acNameFilters.cend() == this->acNameFilters.find(
                name)) {
                return;
            }
        }
...
{
    BoostScopedReadLock readLock(this->acNameFiltersRwlock);
    return this->acNameFilters.cend()
        != this->acNameFilters.find(acNameFilter);
}
...
void Logger::shrinkToFit() noexcept
{
    ...
    std::string tmpFilename;
    {
        ...
        off_t const size = GetFileSize(this->path);
        if (size < 0) {
            // Fail ignore
        ...
        // Current size => max / 2
        std::cout << "Logger::shrinkToFit: will limitSize " << size
            <<" to half of max " << this->maxSize << "\n";
        // Open to read last maxSize / 2 bytes
        ::uchat::File currentLog(this->path);
        int32_t code = currentLog.open(FileOpenConfig{
            PosixFileAccessMode::ReadOnly });
        if (code < 0) {
        ...
        ::uchat::File tmpFile(tmpFilename);
        if (tmpFile.open(FileOpenConfig{
            PosixFileAccessMode::WriteOnly,
            SimplifiedFileOpenFlag::Create,
            &modes })
            < 0) {
        ...
        uint64_t wroteBytes;
        std::tie(code, wroteBytes) = currentLog.traverse(
            doWrite2Tmp, kBigPerReadBytes, this->maxSize / 2);
        currentLog.close();
        tmpFile.close();
        if (tmpFile.open(FileOpenConfig{ PosixFileAccessMode::ReadOnly })
            < 0) {
            std::cerr << "Logger::shrinkToFit: cannot open tmp file\n";
            goto rmtmp;
        }
        if (code >= 0) {
            ...
            this->log = nullptr;
            // Move
            if (currentLog.open(FileOpenConfig{
                PosixFileAccessMode::WriteOnly,
                SimplifiedFileOpenFlag::Create,
                &modes })
                < 0) {
            ...
            auto const doWrite2Log = [&code, &currentLog](
                uint8_t const* const data, uint32_t const size) -> bool {
                uint64_t w;
...
std::string ToString(LogLevel const& logLevel) noexcept
{
    switch (logLevel) {
...
#   if 0
    ...
    ss << tp.tv_sec << ".";
    char buf[16];
    ::snprintf(buf, 10, "%09ld", tp.tv_nsec);
    return ss.str() + buf;
#   else
    // -> Xx yyyy-MM-dd hh:mm:ss.nnnnnnnnn
    return MakeTzLocalNanoTimestamp();
#   endif
}
}
